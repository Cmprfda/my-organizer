...
const HELP = [
    ...
    },
    {
        ...
        items: [
            ...
            ["Por omissão o assistente é <strong>determinístico</strong>: percebe um conjunto de frases conhecido (escreve <code>ajuda</code> para o ver) e, fora disso, procura em tudo o que está aberto. Quem quiser respostas a perguntas escritas à sua maneira pode ligar o <strong>motor do modelo</strong> em <strong>Definições → Assistente</strong> (só no computador onde a app corre), com o botão <strong>Testar</strong> ao lado para confirmar que responde; precisa do pacote <code>anthropic</code> instalado. Mesmo ligado, o modelo só sabe o que a app tem aberto naquele momento: nunca lê a folha nem o OneDrive.",
                "By default the assistant is <strong>deterministic</strong>: it understands a known set of phrases (type <code>help</code> to see them) and otherwise searches everything that is open. If you want answers to questions phrased your own way, you can switch on the <strong>model engine</strong> in <strong>Settings → Assistant</strong> (only on the computer running the app), with the <strong>Test</strong> button next to it to confirm it answers; it needs the <code>anthropic</code> package installed. Even switched on, the model only knows what the app has open at that moment: it never reads the sheet or OneDrive."],
