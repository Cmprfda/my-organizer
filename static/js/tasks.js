...
function render() {
  ...
  $("refresh").textContent = pending ? `Push (${pending})` : t("btn_refresh");
  ...
  function todoAddBtn(r, ri) {
    ...
    return todoHas("task", title, ref) ? ""
      : `<button type="button" class="todoActionBtn" data-todoadd="${ri}" title="${t("todo_add_click")}">+ TODO</button>`;
  }
    ...
    `<tr draggable="true" title="${t("t_drag")}">` + headers.map((_, i) => {
      const cell = (() => {
        ...
        if (useCompact && i === 4) {
          ...
          const title = (n && n.updated ? `${t("t_updated")} ${esc(n.updated)} — ` : "") + t("t_edit_note");
          return `<td class="execCell" data-ri="${ri}" title="${title}">${inner}</td>`;
        }
        if (useCompact && i === 3) {
          const [todo, obs] = String(c).split("\u001F");
          return `<td>${esc(todo)}${obsHtml(obs || "", ri)}</td>`;
        }
        return `<td>${esc(c)}</td>`;
      })();
