...
- **`run-hidden.vbs`:** Launched by the desktop shortcut (`wscript.exe run-hidden.vbs`) instead of `cmd.exe run.bat` directly, so no console window appears — it runs `run.bat` with `WindowStyle 0` (hidden) and doesn't wait for it to exit. Double-clicking `run.bat` itself still shows the console (used for dev/debugging).
