...
- **Data freshness:** the UI polls `/api/modified` every 20 s (cheap `lastModifiedDateTime`/mtime call) and reloads as soon as the `stamp` changes; the 2-minute `load()` cycle stays as a fallback. The **Atualizar** button sends `fresh=1`, which drops `_RAW_CACHE`/`_LAST_GOOD`/`_ADMIN_CACHE` and re-resolves the OneDrive item — a cold read, like opening the file for the first time. The same 20 s tick also drives the **per-task notifications** (`static/js/notify.js`): after each reload it compares a snapshot of the **person-scoped** rows (always `all=0`, reusing the payload already loaded when *Ver tudo* is off) keyed by `xlrow`, and stacks one card per changed row in `#notifyStack` (bottom-right, `notify.css`). The snapshot holds `row_meta[].orig` (the sheet's real values), so local overrides (✎) never raise notifications; the first read of a session/book/sheet only seeds it, never notifies.
