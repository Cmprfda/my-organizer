...
- **Local JSON State (NEVER include in releases):** `status_overrides.json`, `notes.json`, `ccrs.json`, `todo.json`, `notepad.json`, `notepad_images\`, `bug_reports.json`, `tracker.log`, `graph_config.json`, `graph_token.json`, `workbooks.json`, `feedback_pending\` (feedback estagiado localmente até ser entregue na pasta partilhada via Graph ou na cópia sincronizada; `cswaios.feedback.flush_pending()` entrega-o depois).
