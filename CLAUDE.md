...
- **`run.bat`:** Double-click starter: detects Python, installs `openpyxl` (required) and `pywebview` (optional — failure to install only downgrades the UI to the system browser, never blocks startup), kills prior process on port 8765, and launches.
- **`setup.bat`:** Initial setup script (winget Python if missing, same dependency installs as `run.bat`, creates desktop shortcut).
- **`My Organizer.vbs`:** Launched by the desktop shortcut (`wscript.exe "My Organizer.vbs"`) instead of `cmd.exe run.bat` directly, so no console window appears — it runs `run.bat` with `WindowStyle 0` (hidden) and doesn't wait for it to exit. Double-clicking `run.bat` itself still shows the console (used for dev/debugging).
- **UI launch (`server.open_ui`):** the server always binds and runs in a background daemon thread; the main thread then either opens a native window via `pywebview` (`private_mode=False` so `localStorage`, e.g. the theme preference, persists across launches) or, if `pywebview` isn't installed (e.g. right after an auto-update, since the update restart calls `sys.executable app.py` directly and skips `run.bat`'s pip install step), falls back to `webbrowser.open`. Closing the native window ends the process (the server thread is a daemon). `--no-browser` (used by the `/api/update` self-restart and headless test instances) skips opening any UI and just idles, since an existing window/tab is expected to reload itself.
